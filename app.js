  ...
  try {
    ...
    if (storedData) {
      try {
        ...
        console.log('Загружены подписки для миграции', localSubscriptions.length);
      } catch (parseError) {
        console.error('Ошибка при парсинге данных для миграции', parseError.message);
        return false;
      }
    } else {
    ...
    // Если нет подписок для миграции, выходим
    if (!localSubscriptions || localSubscriptions.length === 0) {
      return false;
    }
    
    // Преобразуем в формат Supabase, добавляя user_id к каждой подписке
    ...
    for (const sub of formattedSubscriptions) {
      try {
        const saved = await window.saveSubscription(sub);
        if (saved) {
        ...
        console.error('Ошибка при сохранении подписки в Supabase', e.message);
      }
    }
    
    // Если хотя бы одна подписка успешно мигрирована
    if (savedSubscriptions.length > 0) {
      console.log('Миграция завершена, сохранено ' + savedSubscriptions.length + ' подписок');
      
      // Обновляем глобальный массив подписок
      subscriptions = savedSubscriptions;
      
      // Создаем резервную копию
      try {
        localStorage.setItem('subscriptions_old', storedData);
        console.log('Создана резервная копия в localStorage (subscriptions_old)');
      } catch (e) {
        // Игнорируем ошибки при создании резервной копии
      }
      
      return true;
    } else {
      console.error('Миграция в Supabase не удалась, используем локальные данные');
      subscriptions = localSubscriptions;
      return false;
    }
  } catch (error) {
    console.error('Ошибка при миграции подписок', error.message);
    return false;
  }
} 